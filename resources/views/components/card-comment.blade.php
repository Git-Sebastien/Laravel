      ...
      <div class="card card-comment">
          <div class="img-user-comment">
            @foreach ($user as $item)
              ...
              @elseif($comment->author == $item->name && empty($item->image_path))
                <img src="{{ asset("images/avatar/$item->default_image") }}" alt="" class="p-2 rounded-circle">
              @endif
            @endforeach
        ...
        <div class="card-body">
          ...
          @if (!empty(Auth::user()->id) &&  Auth::user()->id == $comment->user_id || !empty(Auth::user()) && Auth::user()->role_id == 1)
            ...
            <form action="{{ route('destroyComment',$comment->id) }}" method="post">@csrf  
              <button type="submit" class="card-link">{{ __('Supprimer') }}</button>
            </form>  
          @endif
        </div>
      </div>
