...
@else
    ...
    <x-card-post title="{{ Str::of($post->title)->toHtmlString() }}" content="{{ Str::of(Str::limit($post->content,140))->ToHtmlString() }}"  author="{{ Str::of($post->author)->ToHtmlString }}" route="{{ route('posts.show',$post->id)}}" />
    @endforeach
@endif
